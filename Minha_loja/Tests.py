...
def main(page: ft.Page):
    ...
    def criar_item_carrinho(item, atualizar_contador, atualizar_total, page, carrinho_itens):
        quantidade_text = ft.Text(str(item["quantidade"]), size=14, weight="bold")
        subtotal_text = ft.Text(f"Subtotal: R$ {item['preco'] * item['quantidade']:.2f}", size=12, color="#5E3264")
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ...
                    ft.Column([
                        ft.Text(item["nome"], size=12, weight="bold", color=ft.colors.BLACK),
                        ft.Text(f"Preço: R$ {item['preco']:.2f}", size=12, color="#5E3264"),
                        ft.Row([
                            ft.IconButton(ft.icons.REMOVE, on_click=lambda e: atualizar_quantidade(e, -1)),
                            quantidade_text,
                            ft.IconButton(ft.icons.ADD, on_click=lambda e: atualizar_quantidade(e, +1)),
                            subtotal_text
                        ])
                    ])
                ]),
